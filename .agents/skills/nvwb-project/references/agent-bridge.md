...
**Outer agent** (the agent on the local machine, or the user) reads the request, takes action, and writes results to `/project/agent-bridge/outer.md`.
...
The rules above are documentation — agents follow them but nothing enforces them. Agent **hooks** (Codex and Claude Code both support them) can enforce these constraints at the tool level, blocking disallowed actions before they execute.

The skills repo ships hook scripts at `nvwb-project/hooks/`. Copy them into your project's `agent-bridge/hooks/` directory:

```bash
cp -r ~/.codex/skills/nvwb-project/hooks/ <project-path>/agent-bridge/hooks/
```
...
- Blocks reads of sensitive files (`.env`, `~/.claude.json`, `~/.codex/auth.json`, credential files) — note: read-blocking is enforced under Claude Code; current Codex does not fire `PreToolUse` for `Read` (see the Codex caveat under *Wiring up the hooks*)
...
Hooks scrub sensitive environment variables from the agent's shell commands (wrapping them with `env -u`). Variable names are collected from three sources, in priority order:
...
Codex reads hooks from `[hooks]` in `config.toml` or a `hooks.json`, at either the global
(`~/.codex/`) or project (`<repo>/.codex/`) level. Project hooks load only for trusted
projects. Codex's PreToolUse wire format is the same as Claude Code's (`hookSpecificOutput`
→ `hookEventName: "PreToolUse"` → `permissionDecision` → `permissionDecisionReason` /
`updatedInput`), so the **same** `inner-agent-*.sh` / `outer-agent-*.sh` scripts work
unchanged — only the registration differs.

**Inner agent** (Codex in the container) — add to `~/.codex/config.toml` (or `.codex/config.toml`):
...
type = "command"
command = "/project/agent-bridge/hooks/inner-agent-bash.sh"
timeout = 30
statusMessage = "Checking command against bridge policy"

[[hooks.PreToolUse]]
matcher = "^(apply_patch|Edit|Write)$"
[[hooks.PreToolUse.hooks]]
type = "command"
command = "/project/agent-bridge/hooks/inner-agent-files.sh"
```

**Outer agent** (Codex on the local machine) — add to `~/.codex/config.toml`:
...
[[hooks.PreToolUse.hooks]]
type = "command"
command = "$(git rev-parse --show-toplevel)/agent-bridge/hooks/outer-agent-bash.sh"

[[hooks.PreToolUse]]
matcher = "^(apply_patch|Edit|Write)$"
[[hooks.PreToolUse.hooks]]
type = "command"
command = "$(git rev-parse --show-toplevel)/agent-bridge/hooks/outer-agent-files.sh"
```

> **Codex coverage caveat:** in current Codex, `PreToolUse` fires for **Bash, `apply_patch`
> (Edit/Write), and MCP tools** — but **not** the `Read` tool. So the Bash command policy and
> the file-write restrictions are enforced under Codex, but `inner-agent-read.sh`
> (sensitive-file read blocking) does **not** fire. If read-blocking matters, run the inner
> agent under Claude Code (which fires `Read` PreToolUse), or rely on `sandbox_mode` /
> `approval_policy` in `config.toml`. The bridge's Bash-level protections (nvwb redirect,
> env-dump block, sensitive-var scrubbing) are fully enforced under Codex.
...
Requires `jq` and `bash`. Tests cover the cross-harness (Claude Code / Codex) and Cursor hook scripts.

---

## Bootstrapping the Outer Agent

The outer agent (on the local machine) needs to know the bridge exists. Add an `AGENTS.md` (read by Codex) or `CLAUDE.md` (read by Claude Code) file in the project root that points it at `agent-bridge/`:
...
```bash
cp -r ~/.codex/skills/cursor-rules/ai-workbench /project/.cursor/rules/
```
