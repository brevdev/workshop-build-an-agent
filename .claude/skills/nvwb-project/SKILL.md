...
This skill activates when Claude is working inside an AI Workbench project container (detected by `.project/spec.yaml` existing in or above the current working directory).
...
- Changes to compose files require `nvwb compose down` + `nvwb compose up` on the host — request this via the bridge if running as inner Claude
