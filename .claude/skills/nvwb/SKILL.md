...
The `nvwb` CLI always runs on the **local machine** — the user's laptop or desktop. From there it manages contexts, which may be local (container on the same machine) or remote (container on a server or cloud instance accessed via SSH). Claude using this skill is on the local machine.
...
- `references/claude-in-container.md` — Running Claude Code inside the container, credential persistence, and the host↔container bridge pattern
