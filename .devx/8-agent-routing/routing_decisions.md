...
Now the router opens the envelope. In capability mode, `llm_classifier` hands the request to a small **judge** model that scores how hard the task looks; the score is compared against `base_threshold`, and anything above it goes to the strong target while everything below rides the cheap one.

The threshold is the part people misread. `base_threshold` isn't a magic number somebody tuned for you — it's a **policy dial**, and where you set it says which mistake you'd rather make. Push it down and more traffic escalates: you spend more, you fail less. Push it up and you save more and drop more tasks on the floor. Exercise 5 sweeps it across three values and watches the Pareto curve move.

Two more knobs exist because a classifier that re-decides every turn is its own failure mode. `session_affinity` pins a session to the tier it started on, so the agent doesn't flap between brains mid-conversation. `message_hash_fallback` reuses the previous verdict when the judge doesn't return a parseable one. Both are cheap insurance against the thing content routing does worst: changing its mind.
...
There's a whole class of evidence you're already producing and throwing away. An agent loop emits tool calls and tool results every turn, and that stream says a lot about where the agent *is*: three searches and two file reads in a row is exploration; a long synthesis turn after the reads is implementation. `stage_router` scores that recent trajectory and picks the tier — with no second model call, because the signals were already in the request.
