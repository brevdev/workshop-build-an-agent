...
Answer key — solved version of hermes/client.py.
...
def chat(messages, tools=None, temperature=None, max_tokens=None) -> dict:
    ...
    # ── TODO: Exercise 1.1 — build the request payload (SOLVED) ──
    payload = {
        "model": config.MODEL_NAME,
        "messages": messages,          # this list IS the context window
        "temperature": temperature,
        "max_tokens": max_tokens,
    }
    if tools:
        payload["tools"] = tools       # same schema list Module 1 passed to the SDK
    ...
    # ── TODO: Exercise 1.2 — send the request and parse the reply (SOLVED) ──
