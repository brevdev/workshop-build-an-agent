"""The Harness Face-Off (Exercise 4).
...
def run_hermes(steps, restart_between) -> list:
    ...
    """
    out = []
    # ── TODO: Exercise 4 — drive your own harness ──
    # 1. Make one Harness with gates auto-approved (this is unattended):
    #        agent = Harness(auto_approve=True)
    # 2. For each step, call agent.send(step) and append the reply to `out`.
    # 3. If restart_between is True, create a fresh Harness(auto_approve=True)
    #    before each step AFTER the first, so only MEMORY.md carries facts over.
    agent = ...  # Hint: Harness(auto_approve=True)
    for i, step in enumerate(steps):
        if restart_between and i > 0:
            agent = ...  # Hint: a brand-new Harness(auto_approve=True)
        out.append(...)  # Hint: agent.send(step)
    return out
