# `using_nemoclaw.md` Rewrite — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild Module 6's `using_nemoclaw.md` around agent-safety concepts, with 6 exercises mapped to `why_nemoclaw.md`'s four layers, and add four motivating "unsecured baseline" probes to `setup_openclaw.md`.

**Architecture:** Two-file docs rewrite. `setup_openclaw.md` gains a short "Know Your Baseline" section at the end (four probes). `using_nemoclaw.md` is fully replaced with a 4-section / 6-exercise structure. All Python scaffolding (`agent_safety.py`, policies, test fixtures) stays untouched — only the narrative and pedagogical ordering change.

**Tech Stack:** Markdown with GitHub Flavored features, Mermaid diagrams, `<details><summary>` collapsibles, JupyterLab `<button onclick="...">` launchers, and `<!-- fold:break -->` slide markers per the existing workshop conventions.

---

## File Structure

Two files touched, one spec referenced throughout:

| Path | Action | Purpose |
|---|---|---|
| `.devx/6-agent-safety/setup_openclaw.md` | **Modify** | Append "Know Your Baseline" section before "What's Next" |
| `.devx/6-agent-safety/using_nemoclaw.md` | **Rewrite (replace body)** | New 4-section / 6-exercise structure |
| `docs/superpowers/specs/2026-04-16-using-nemoclaw-rewrite-design.md` | **Reference** | Source of truth for exercise structure and content |

**No code files modified.** No new test fixtures. Existing `code/6-agent-safety/` layout is preserved.

---

## Workshop Conventions to Preserve

All tasks below must respect these site-wide conventions:

1. **Slide markers** — `<!-- fold:break -->` on its own line to break between slides.
2. **Terminal button** — `<button onclick="openNewTerminal();"><i class="fas fa-terminal"></i>terminal</button>` to launch a new terminal.
3. **File open button** — `<button onclick="openOrCreateFileInJupyterLab('<path>');"><i class="fa-brands fa-python"></i> <label></button>` for JupyterLab file-open.
4. **Line jump button** — `<button onclick="goToLineAndSelect('<path>', '<search-text>');"><i class="fas fa-code"></i> <label></button>` for jumping to a specific code location.
5. **Launcher button** — `<button onclick="launch('NemoClaw Client');"><i class="fa-solid fa-rocket"></i> NemoClaw Client</button>` to open the Streamlit client.
6. **Collapsibles** — `<details><summary><strong>Title</strong></summary>...</details>` for deep dives. The summary must be bold via `<strong>` when it's a concept name.
7. **Mermaid theme** — use `theme: 'base'` with `primaryColor: '#eaf6e0'` and `secondaryColor: '#eaf6e0'` on a white background (NVIDIA green).
8. **Back-links** — plain Markdown links like `[Why NemoClaw: Principles and Layers](why_nemoclaw)` (no `.md` extension — the site renderer strips it).
9. **Right-aligned robot image** — `<img src="_static/robots/supervisor.png" alt="..." style="float:right;max-width:300px;margin:25px;" />` for page headers.
10. **Callout style** — `> Blockquote` for "what just happened" explanations and forward-references.

---

## Content Authority Notes

Cross-file facts the plan depends on:

- **Four layers** per `why_nemoclaw.md`: Network → Filesystem → Process → Inference.
- **Python exercise file**: `code/6-agent-safety/agent_safety.py` has TODO comments for Exercises 1–5 (referred to as M6.B1–M6.B5 in this plan).
- **Policy fixtures**: `code/6-agent-safety/policies/{baseline_permissive,research_assistant}.yaml`.
- **Test fixtures**: `code/6-agent-safety/test_data/{redteam_probes,mixed_sensitivity_corpus,safety_eval_dataset}.json`.
- **Mock leaky agent**: `openclaw_wrapper.py::_create_mock_agent()`.
- **Privacy Router truth**: operator-chosen backend + credential isolation; does NOT inspect content. Content classification is built on top.
- **Ollama reference**: `https://build.nvidia.com/spark/nemoclaw/instructions` Step 2 for DGX Spark path.

---

# Task List

## Task 1: Append "Know Your Baseline" section to `setup_openclaw.md`

**Files:**
- Modify: `.devx/6-agent-safety/setup_openclaw.md` — insert a new section immediately before the existing `## What's Next` section (currently around line 166)

**Goal:** Add four probes that let the reader feel the vulnerability of vanilla OpenClaw, each forward-referencing the upcoming NemoClaw exercise.

- [ ] **Step 1: Read the current `setup_openclaw.md` to confirm insertion point**

Run: `Read` on `.devx/6-agent-safety/setup_openclaw.md` around line 160-172
Expected: See current "What's Next" section starting ~line 166.

- [ ] **Step 2: Insert the new section before "What's Next"**

Insert exactly this content (follows the fold-break convention, uses the existing button styling, and preserves narrative voice):
...
- [ ] **Step 3: Verify insertion by re-reading the section around lines 165-260**

Run: `Read` on `.devx/6-agent-safety/setup_openclaw.md` around the inserted region.
Expected: New section is present, followed by the existing `## What's Next` section. Slide markers (`<!-- fold:break -->`) correctly separate each probe. No broken Markdown or missing `</button>` tags.

- [ ] **Step 4: Visual/cross-reference check**

Confirm the following textual cross-references appear exactly:
- "Exercise 1" forward-ref in Probe 1
- "Exercise 3" forward-ref in Probe 2
- "Exercise 4" forward-ref in Probe 3
- "Exercise 6" forward-ref in Probe 4

These MUST match the exercise numbering in `using_nemoclaw.md` (written in Tasks 3–8 below).

- [ ] **Step 5: Commit**

```bash
git add .devx/6-agent-safety/setup_openclaw.md
git commit -m "Add 'Know Your Baseline' probe section to setup_openclaw

Four short probes (Phone Home, Read the Diary, Spill the Keys, Poison
the Memory) that demonstrate vanilla OpenClaw's vulnerabilities and
forward-reference the hardening exercises in using_nemoclaw.md."
```

---

## Task 2: Replace `using_nemoclaw.md` — write the opening section

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — rewrite from scratch using `Write` to replace the whole file in stages (this task covers the header, intro, and roadmap; later tasks append each exercise)

**Goal:** Write the page header, the recap/roadmap intro, and the section heading for Section 1. The page will be built up incrementally across Tasks 2–8 using `Edit` to append; Task 2 uses `Write` once to establish the file skeleton.

- [ ] **Step 1: Write the new file skeleton**

Replace the entire file with this content (uses `Write` since it's a full replacement):
...
Run: `Read` on `.devx/6-agent-safety/using_nemoclaw.md`
Expected: See exactly the content above, ending with the `<!-- fold:break -->` marker ready for Task 3 to append.

- [ ] **Step 3: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Rewrite using_nemoclaw.md: opening, recap, and roadmap

Replaces the current Part A/Part B split with a single arc organized
around the four enforcement layers from why_nemoclaw.md. Each of the
six exercises follows a recall/observe/harden/validate pattern."
```

---

## Task 3: Append Section 1 — Exercise 1 (Stop the agent from phoning home)

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — append after the existing `<!-- fold:break -->` at the end of Task 2's content

**Goal:** Write Section 1's opening and Exercise 1 in full — deny-by-default network policy, policy validator Python sidekick.

- [ ] **Step 1: Append Section 1 + Exercise 1**

Use `Edit` with `old_string = "<!-- fold:break -->"` (the trailing one from Task 2) and `new_string` = the trailing marker plus the following content. Or, more safely, use `Edit` with `old_string = "commands run in a separate terminal outside the sandbox.\n\n<!-- fold:break -->"` to anchor on the end of Task 2. Append:
...
- [ ] **Step 2: Verify the content was inserted correctly**

Run: `Read` on `.devx/6-agent-safety/using_nemoclaw.md`, focusing on the last ~180 lines.
Expected: Section 1 heading, Exercise 1 with 5 steps, deny example, policy YAML, Python sidekick with collapsible help, and closing callout. All `<!-- fold:break -->` markers on their own lines. No broken Markdown.

- [ ] **Step 3: Spot-check the sidekick code against `agent_safety.py`**

Run: `Read` on `code/6-agent-safety/agent_safety.py` around line 114–162.
Expected: The `load_and_validate_policy` function body matches the sidekick solution above (variable names: `policy_data`, `violations`, `has_critical`).

- [ ] **Step 4: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Add Exercise 1: stop the agent from phoning home

Section 1 opener + first exercise. Walks through a deny-by-default
403, reading the baseline policy, writing a new network_policies YAML,
hot-reloading with openshell policy set, and the Python sidekick that
catches weak policies in CI/CD."
```

---

## Task 4: Append Exercise 2 (Not all allow-rules are equal)

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — append after Task 3's closing fold-break

**Goal:** Show the L7 vs L4 gotcha — the `protocol: rest` field turns per-method inspection on; omitting it leaves you with coarse TCP passthrough.

- [ ] **Step 1: Append Exercise 2**

Append the following. Anchor on the last marker inserted in Task 3 (`"What you just learned:** deny-by-default is the posture, hot-reload is the operational affordance, and programmatic validation is the safety net that catches policy regressions before they hit production.\n\n> <!-- fold:break -->"` or similar — use whatever end-of-file marker exists).
...
Run: `Read` on the file, look for "Exercise 2" and confirm the three subsections (L7 POST denial, TCP bypass, per-binary scoping).
Expected: Clean structure, no missing closing tags.

- [ ] **Step 3: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Add Exercise 2: L7 vs L4 policy enforcement

Teaches the 'protocol: rest' gotcha (omit it and access: read-only
becomes meaningless) and per-binary scoping (rules for curl don't
cover python3). Both are real subtleties documented in OpenShell's
policy schema."
```

---

## Task 5: Append Section 2 — Exercise 3 (Make containment irrevocable)

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — append after Task 4

**Goal:** Combine Filesystem (Landlock) + Process (seccomp + non-root + dropped caps) into a single exercise with two subsections. Both are kernel-level, both are static, both demonstrate "irrevocable by design."

- [ ] **Step 1: Append Section 2 opening + Exercise 3**
...
Run: `Read` on the file; find Section 2 + Exercise 3 with Part A (5 steps) + Part B (4 steps + collapsible).
Expected: clean structure, no unclosed `<details>` tags.

- [ ] **Step 3: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Add Exercise 3: filesystem + process kernel-level containment

Combines Landlock (Layer 2) and seccomp + non-root + dropped caps
(Layer 3) into one exercise. Demonstrates irrevocability with bypass
attempts, shows static-vs-dynamic contrast with Exercise 1, extends
the policy validator to catch root-writable paths."
```

---

## Task 6: Append Section 3 — Exercise 4 (Remove the keys from the agent)

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — append after Task 5

**Goal:** Show credential isolation via `inference.local`, then surface the "both layers required" callout by showing that network policy must also deny direct provider hosts.

- [ ] **Step 1: Append Section 3 opening + Exercise 4**
...
- [ ] **Step 2: Verify** — Read the file, confirm the section 3 opener + Exercise 4 + both-layers callout.

- [ ] **Step 3: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Add Exercise 4: credential isolation + both-layers-required

Demonstrates inference.local credential injection, then the key
teaching moment: credential isolation alone does NOT prevent
exfiltration if network policy allows direct provider hosts.
Defense-in-depth requires the Network layer closing this path."
```

---

## Task 7: Append Exercise 5 (Route sensitive queries locally)

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — append after Task 6

**Goal:** Reframe Privacy Router honestly, demonstrate provider hot-swap, introduce content classifier as the *thing you build on top*.

- [ ] **Step 1: Append Exercise 5**
...
- [ ] **Step 2: Verify** — Read the file, confirm Exercise 5 with three paths (Spark/small/no-Ollama), classifier sidekick, limitations table.

- [ ] **Step 3: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Add Exercise 5: Privacy Router + content classifier pattern

Reframes Privacy Router as operator-chosen routing + credential
isolation (factually accurate per OpenShell docs), demonstrates
provider hot-swap with three environment-dependent paths, and
positions the content classifier (M6.B2) as the pattern you build
on top of the Privacy Router primitive."
```

---

## Task 8: Append Section 4 — Exercise 6 (Continuous safety evaluation)

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — append after Task 7

**Goal:** Capstone exercise that combines red-team probing (M6.B3), LLM-as-judge (M6.B4), and end-to-end safety suite (M6.B5) into three phases of one exercise.

- [ ] **Step 1: Append Section 4 + Exercise 6**
...
- [ ] **Step 2: Verify** — Read the file, confirm Section 4 + Exercise 6 with three phases.

- [ ] **Step 3: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Add Exercise 6: continuous safety evaluation capstone

Three-phase capstone combining red-team probes (M6.B3), LLM-as-judge
scoring (M6.B4), and end-to-end safety suite (M6.B5). Consolidates
what were four separate Python exercises in the old structure into
a single cohesive workflow, scored with a weighted aggregate usable
in CI/CD."
```

---

## Task 9: Append module wrap-up and "What to Explore Next"

**Files:**
- Modify: `.devx/6-agent-safety/using_nemoclaw.md` — append after Task 8

**Goal:** Preserve the module wrap-up table and external links from the current page; add a closing paragraph that reinforces the "agent safety as a discipline" framing.

- [ ] **Step 1: Append closing content**
...
- [ ] **Step 2: Verify** — Read the end of the file, confirm wrap-up table + explore-next list + congratulations.

- [ ] **Step 3: Commit**

```bash
git add .devx/6-agent-safety/using_nemoclaw.md
git commit -m "Add module wrap-up and 'What to Explore Next' closing

Preserves the arc-across-all-six-modules table from the prior
version, plus external links for going deeper. Closes the page with
'agent safety as a discipline' framing consistent with the module's
new focus."
```

---

## Task 10: Full-page consistency and cross-reference validation

**Files:**
- Read: `.devx/6-agent-safety/setup_openclaw.md`, `.devx/6-agent-safety/using_nemoclaw.md`
- No modifications unless issues are found

**Goal:** Catch any broken internal links, inconsistent exercise numbering, missing fold-break markers, or broken HTML.

- [ ] **Step 1: Read `using_nemoclaw.md` top-to-bottom**

Run: `Read` on `.devx/6-agent-safety/using_nemoclaw.md` with no offset
Expected: file is ~28–35 KB, reads cleanly as a single narrative.

- [ ] **Step 2: Check cross-references between the two pages**

Run: `Grep` for `"Probe [1-4]"` in `.devx/6-agent-safety/using_nemoclaw.md`
Expected: each of Probes 1, 2, 3, 4 is referenced in the intended exercise (1, 3, 4, 6 respectively).

Run: `Grep` for `"Exercise [1-6]"` in `.devx/6-agent-safety/setup_openclaw.md`
Expected: Exercise 1, 3, 4, 6 all appear in the "Later:" callouts in the four probes.

- [ ] **Step 3: Check fold-break markers**

Run: `Grep` on `<!-- fold:break -->` in both files, count occurrences.
Expected: `setup_openclaw.md` has more fold-breaks than before the change (roughly +5 from the new section). `using_nemoclaw.md` has ~35–50 depending on how many sub-steps needed separation. Every section header and every Step N in an exercise should have a fold-break before it.

- [ ] **Step 4: Check for broken HTML**

Run: `Grep` for `<details>` and `</details>` in `using_nemoclaw.md`
Expected: equal counts. If not, a collapsible is unclosed — find and fix.

Run: `Grep` for `<button` without matching `</button>` — scan visually.
Expected: all `<button>` tags paired with `</button>`.

- [ ] **Step 5: Run the workshop doctor / link check (if available)**

Run:
```bash
grep -rE "\[(why_nemoclaw|setup_openclaw|setup_nemoclaw|using_nemoclaw)\]" .devx/6-agent-safety/
```
Expected: each cross-reference link appears in at least one other page (ensures navigation works).

- [ ] **Step 6: If issues found, fix and commit separately**

If any inconsistency is found, use `Edit` on the offending file to fix, then:

```bash
git add .devx/6-agent-safety/
git commit -m "Fix consistency issues from final review pass"
```
...
Once everything is verified, optionally tag the branch state:

```bash
git log --oneline -8
```

Expected: 8 commits from Tasks 1–9 plus (optionally) a final fixup from Step 6. Branch ready for review.

---

## Self-Review Checklist

After all tasks are complete, sanity check:

- [ ] `setup_openclaw.md` has the 4 probes, each forward-referencing the correct exercise number
- [ ] `using_nemoclaw.md` has exactly 6 exercises across 4 sections
- [ ] Every exercise has layer tags (italicized, at the top)
- [ ] Every Python sidekick references an existing TODO in `agent_safety.py`
- [ ] Privacy Router is described as operator-chosen routing + credential isolation (not content classification)
- [ ] "Both layers required" callout is present in Exercise 4
- [ ] Module wrap-up table + "What to Explore Next" closing are preserved
- [ ] All `<button>`, `<details>`, and `<!-- fold:break -->` syntax is valid
- [ ] Cross-references between the two pages resolve correctly
